...
- Keep messages short and simple — a single lowercase line describing the change, matching the existing `git log` style.
- Never add a `Co-Authored-By` trailer. Do not credit Claude as a co-author.
...
- **Designs** (`src/designs/`) — predefined layout templates (`top8er`, `minimal`, `squares`, `kagaribi`). Each design is a typed object describing all canvas elements. The kagaribi template uses `altCharacterImage` with `flow: "column"` and `rows: 6` to display alt character icons in a top-right vertical grid that grows leftward.
